...
rule all:
    input:
        ...
        expand("output/cdifficile/astar_recgraph/full.csv")
    output:
...
rule run_recgraph_a_star:
    input:
        ...
        gfa = "data/cdifficile/slpa-basis.pggb.gfa",
    output:
        gaf = "output/cdifficile/astar_recgraph/split.{seq}.gaf",
    threads: 1
    resources:
        mem="12GB"
    shell:
...
def aggregate_astar_input(wildcards):
    checkpoint_output = checkpoints.split_seqs.get(fa='simulated').output[0]
    ret = expand(
        "output/cdifficile/astar_recgraph/split.{seq}.gaf",
        seq=glob_wildcards(os.path.join(checkpoint_output, "split.{seq}.fa")).seq
    )
    return sorted(ret)

rule merge_recgraph_a_star:
    input:
        aggregate_astar_input
    output:
        "output/cdifficile/astar_recgraph/full.gaf",
    threads: 1
    shell:
        """
        cat {input} > {output}
...
rule make_csv_astar:
    input:
        gaf = "output/cdifficile/astar_recgraph/full.gaf",
    output:
        csv = "output/cdifficile/astar_recgraph/full.csv",
    run:
        out = open(output.csv, 'w+')
        print("ReadName,RecPaths,RecPos,Score,Displacement", file=out)
