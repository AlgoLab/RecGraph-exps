...
for gene in genes:
    for err in error_levels:
        output_dir = f"output/HLA/genes/{gene}/reads_{err}_split"
...
rule run_recgraph:
    input:
        fa="output/HLA/genes/{gene}/reads_{err}_split/{read_file}.fa",
        gfa="output/HLA/genes/{gene}/graph.gfa",
        rg = "bin/recgraph"
    log:
        "output/HLA/old_recgraph/{gene}/reads_{err}_split/{read_file}.log"
    output:
        "output/HLA/old_recgraph/{gene}/reads_{err}_split/{read_file}.gaf"
    shell:
...
rule run_recgraph_a_star:
    ...
    log:
        "output/HLA/new_recgraph/{gene}/reads_{err}_split/{read_file}.log"
    output:
        "output/HLA/new_recgraph/{gene}/reads_{err}_split/{read_file}.gaf"
    shell:
